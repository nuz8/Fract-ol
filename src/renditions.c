...
/*   Updated: 2024/07/27 09:25:04 by pamatya          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_mandelbrot(t_fractal *fr)
{
	...
	while (dim[0] < HEIGHT)
	{
		...
		while (dim[1] < WIDTH)
		{
			...
			fr->c.rl = scale_point(tmp.rl, WIDTH, fr->lbound, fr->rbound)  + fr->move_hor;
			fr->c.im = scale_point(tmp.im, HEIGHT, fr->tbound, fr->bbound) + fr->move_ver;
...
void	render_julia(t_fractal *fr)
{
	...
	while (dim[0] < HEIGHT)
	{
		...
		while (dim[1] < WIDTH)
		{
			...
			fr->z.im = (scale_point(tmp.im, HEIGHT, fr->tbound, fr->bbound) * fr->zoom) + fr->move_ver;
			escape_val = blast_off(fr);
			color = scale_color(escape_val, fr->iter_rendition, RED, PSYCH_VIOLET);
			mlx_put_pixel(fr->img, dim[1], dim[0], color);
			dim[1]++;
